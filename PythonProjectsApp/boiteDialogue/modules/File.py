...
class choiceListe():
	def __init__(self):
		# Liste devant contenir les noms de chaque sheet du fichier excel
		self.names = []

	def choice_liste(self, path:str,liste_file2:list) :
		# lecture fichier
		...
		if condition==False :
			...
			for k in sheet_datas:
				...
				self.names.append({numFeuille:col})
				numFeuille += 1
	...
	def process(self, personNames, list_image):
		# import librairies
		print('\n-----------import librairies------------')
		import dill
		import cv2 
		...
		# assigner à la variable personNames la liste e noms des sheet 
		# afin de faciliter l'appel de la méthode process  
		...
		for image, iname, iid in zip(list_image, self.names, list(range(1,len(self.names)+1))):
