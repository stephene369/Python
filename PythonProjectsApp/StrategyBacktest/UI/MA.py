...
class collectInformation(SmoothScrollArea) :
    ...
    def initGrid(self , file) :
        ...
        self.moisCombo = ComboBox(self) ; self.moisCombo.addItems(["1. mois(30 jous) " , 
            "2. mois(60 jours)" , "3. mois(90 jours)","4. mois(120 jours) ", "Mois de 1 mois"])
        ...
        try :
            if Path(file).exists()==True :
                df = read_csv(file)
                self.column_numerique = []
                self.column_object = []
                print(df.columns)
                for d in df.columns :
                    if str(df[d].dtype) == "int64" or str(df[d].dtype) == "float64" :
                        self.column_numerique.append(d)
                        
                for o in df.columns :
                    if str(df[o].dtype) == "object" :
                        self.column_object.append(o)
                        print(o)
            else :
                df = "None"
                self.column_numerique = []
            
            print(self.column_numerique , self.column_object)
                                                                                             
        except Exception : self.column_numerique =[];self.column_object=[] ;df = "None"
        ...
        self.combo1.addItems( self.column_object )
        if "Date" in self.column_object : self.combo1.setCurrentText("Date")
        
        self.combo2 = ComboBox(self)
        self.combo2.addItems( list(self.column_numerique) )
        if "Close" in self.column_numerique : self.combo2.setCurrentText("Close")
        ...
        self.lab = BodyLabel(("parametre rapide : "))
        ...
        self.lab = BodyLabel(("Parametre lente : "))
        self.lente1 = SpinBox(self)
        self.lente2 = SpinBox(self)
        self.lente1.setMinimum(10)
        self.lente2.setMinimum(20)
        ...
        self.minBenefice.setMinimum(10)
        ...
        self.minTransaction.setMinimum(10)
        self.grid.addWidget(self.lab,19,0,1,1,LEFT|CENTER)
        self.grid.addWidget(self.minTransaction,19,0,1,1,RIGHT|CENTER)
        
        self.lab = StrongBodyLabel(("Parametre de pré-visualisation des données "))
        self.grid.addWidget(self.lab,20,0,1,2,LEFT|CENTER)
        
        self.lab = BodyLabel(("Moyenne Rapide : "))
        self.fast = SpinBox(self)
        self.grid.addWidget(self.lab,21,0,1,1,LEFT|CENTER)
        self.grid.addWidget(self.fast, 21,0,1,1,RIGHT|CENTER )
        
        self.lab = BodyLabel(("Moyenne Lente : "))
        self.slow = SpinBox(self)
        self.grid.addWidget(self.lab,22,0,1,1,LEFT|CENTER)
        self.grid.addWidget(self.slow,22,0,1,1,RIGHT|CENTER )
...
class maDataBase(SmoothScrollArea):
    def __init__(self, parent=None):
        ...
        self.lineEditBtn.clicked.connect(self.longPrcess2)
        self.LineEdit.setPlaceholderText("exemple : Rapide>9   and   Lente>=19  or   PositiveP>=90 ")
    ...
    def LongProcess(self) :
        if self.process is None or self.process.isFinished()==True:
            self.process = tableWidgetThread(table=self.tableWidget,btn=self.lineEditBtn )
            self.process.update_signal.connect(self.updateTable)
            self.process.start()

    def longPrcess2(self) :
        if self.process2 is None:
            self.process2 = tableWidgetQueryThread(self.tableWidget , self.LineEdit )
...
class createImageThread(QThread):
    ...
    def __init__(self, df:str , col:str,close:str,debut:int,fin:int,
            rapide:int,lente:int,labView:PixmapLabel,labResisatance:PixmapLabel) :
        super().__init__()
        self.df_path = df 
        self.col = col 
        self.close = close
        ...
        self.resistanceLab = labResisatance
    ...
    def run(self):
        ...
        plt.plot(self.df[self.close].index, self.df[self.close].values , label="close",linewidth=1)
        plt.plot(self.df[self.close].index, self.df[self.close].rolling(self.rapide).mean() , label="fast",linewidth=1)
        plt.plot(self.df[self.close].index, self.df[self.close].rolling(self.lente).mean() , label="low",linewidth=1)
        ...
        im1 = im.resize((1000,600))
        im1.save(filename)
        im.close() ; im1.close()
        
        resistance = Support(df=self.df,col=self.close)
        ...
        ## Update lab Pixmap        
        pixmap = QPixmap(r"resource/MA_Preview.png")
        self.labView.setPixmap(pixmap)
        self.labView.update()
        
        pixmap = QPixmap(r"resource\MA_resistance.png")
        self.resistanceLab.setPixmap(pixmap)
        self.resistanceLab.update()
...
class LongProcessThread(QThread):
    ...
    def run(self):
        ...
        for i in range( self.r1 , self.r2+1 ) :
            for j in range(self.l1 , self.l2):
                if j>i+2 : 
                    ...
                    message1+=f"Transaction Negative {self.strategy.negative}\n"
                    message1+=f"Pourcentage de benefice realiser : {self.resultat} {self.emotion}\n"
                    
                    
                    if (self.infos["min_benef"] <= self.resultat) and ( (j-i) <= self.infos["min_diff"]) and (self.infos["min_transaction"] <= (self.strategy.positive/self.strategy.total)*100) :
                        message2 = message1
                    else : 
                        message2 = "Mauvais parametres"
                    ...
                    if self.strategy.total == 0 :
                        positive_.append(0)
                        negative_.append(0)
                    else :
                        positive_.append(int((self.strategy.positive/self.strategy.total)*100))
                        negative_.append(int((self.strategy.negative/self.strategy.total)*100))
                    rapide.append(i)
                    lente.append(j)
                    positive.append(self.strategy.positive)
                    negative.append(self.strategy.negative)

                    total.append(self.strategy.total)
                    benefice.append(self.strategy.pourcentage_revenu)
                    
                    if self.strategy.pourcentage_revenu <= 10 : stick.append("❌")
                    elif self.strategy.pourcentage_revenu <= 50 : stick.append("✅")
                    else:stick.append("✅✅")
        ...
        df=DataFrame(DATA)
        df.to_csv(r"db/MA_DATABASE.csv")
        conn = sql.connect("db/MA_DATABASE.db")
