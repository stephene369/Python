...
df = pd.read_csv("BRVM-Financials.csv")[100:250]
fig, ax = plt.subplots()
X = df["Date"].values
Y1 = df["Close"].values
Y2 = df["Close"].rolling(10).mean()
Y3 = df["Close"].rolling(30).mean()

line1 = ax.plot(X , Y1,linewidht=1)[0]
line2 = ax.plot(X , Y2)[0]
line3 = ax.plot(X , Y2)[0]
...
ani = animation.FuncAnimation(fig=fig,func=update,frames=100,interval=80)
plt.show()
