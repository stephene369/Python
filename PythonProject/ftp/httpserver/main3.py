...
directory_to_serve = create_media_folder()
...
"""
        ...
        if self.path == '/':
            ...
            for file_name in sorted(os.listdir(directory_to_serve)):
                if os.path.isfile(os.path.join(directory_to_serve, file_name)):
                    self.wfile.write(f'''
...
<div class="file">
    ...
    <h4>
    ({os.path.getsize( os.path.join(directory_to_serve, file_name) )/1000 } Mo)
    </h4>
...
start_port = 2024
port, httpd = find_available_port(start_port)
...
with socketserver.TCPServer(("", port), CustomHandler) as httpd:
    ...
    try:
        ...
        logging.error("Certificat ou clé non trouvés.")
    ...
    logging.info(f"http://{local_ip}:{port}/")
    httpd.serve_forever()
