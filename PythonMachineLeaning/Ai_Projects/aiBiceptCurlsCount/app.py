...
class App :
    ...
    ##
        self.btn_class_one = tk.Button(self.window, text = "Extanded",
                                width=50, command=lambda :self.save_for_class(1))
        self.btn_class_one.pack(anchor=tk.CENTER, expand=True)
    ##
        self.btn_class_two = tk.Button(self.window, text = "Contrated",
                                width=50, command=lambda :self.save_for_class(2))
        self.btn_class_two.pack(anchor=tk.CENTER, expand=True)
    ##
        self.btn_class_two = tk.Button(self.window, text = "Train Model",
    ...
    ##
        ...
        self.reset_btn.pack(anchor=tk.CENTER, expand=True)
    ##
    ...
    def update(self) :
        ...
        if self.extended and self.contrated :
            self.extended, self.contrated = False , False 
            self.rep_counter += 1
    ...
    def save_for_class(self , class_num) :
        ...
        path_ = f"{class_num}/frame{self.counters[class_num-1]}.jpg"
        cv2.imwrite(path_ , cv2.cvtColor(frame , cv2.COLOR_RGB2GRAY))
        img = PIL.Image.open(path_)
