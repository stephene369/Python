...
df = pd.read_csv("data/BRVM-Public-Services.csv")[-730:]
fig, ax = plt.subplots()
...
#line4 = ax.plot(X , Y4,marker='^',color='green',linewidth=1)[0]
#line5 = ax.plot(X , Y5,marker='v',color='red',linewidth=1)[0]
...
ani = animation.FuncAnimation(fig=fig,func=update,frames=730,interval=1)
plt.show()
file = "agriculture"
ani.save(filename=f"{file}.gif", writer="pillow")
#ani.save(filename="agriculture.mp4", writer="ffmpeg")


import moviepy.editor as mp

clip = mp.VideoFileClip(f"{file}.gif")
clip = clip.fx(mp.vfx.speedx, 10)   
clip.write_videofile(f"{file}.mp4")